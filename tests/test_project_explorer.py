...
class ProjectExplorerTests(unittest.TestCase):
    def test_build_artifact_exposes_public_files_and_dbt_relationships(self):
        manifest = {
            ...
            "nodes": {
                "model.south_alabama_tornado_watch.stg_ncei__tornado_events": {"package_name": "south_alabama_tornado_watch", "resource_type": "model", "name": "stg_ncei__tornado_events", "path": "staging/ncei/stg_ncei__tornado_events.sql", "original_file_path": "models/staging/ncei/stg_ncei__tornado_events.sql", "description": "Typed source", "columns": {}, "depends_on": {"nodes": ["source.south_alabama_tornado_watch.ncei.tornado_events"]}, "config": {"materialized": "view"}},
                "test.south_alabama_tornado_watch.not_null_stg": {"package_name": "south_alabama_tornado_watch", "resource_type": "test", "name": "not_null_stg_ncei__tornado_events_event_id", "depends_on": {"nodes": ["model.south_alabama_tornado_watch.stg_ncei__tornado_events"]}},
            },
        }
        catalog = {"nodes": {"model.south_alabama_tornado_watch.stg_ncei__tornado_events": {"metadata": {"schema": "ci_staging", "name": "stg_ncei__tornado_events"}}}}
        results = {"results": [{"unique_id": "model.south_alabama_tornado_watch.stg_ncei__tornado_events", "status": "success"}, {"unique_id": "test.south_alabama_tornado_watch.not_null_stg", "status": "pass"}]}
        with tempfile.TemporaryDirectory() as temporary_directory:
            root = Path(temporary_directory)
            source = root / "models/staging/ncei/stg_ncei__tornado_events.sql"
        ...
        self.assertEqual(artifact["files"][0]["path"], "models/staging/ncei/stg_ncei__tornado_events.sql")
        model = next(node for node in artifact["nodes"] if node["resourceType"] == "model")
        self.assertEqual(model["relation"], "ci_staging.stg_ncei__tornado_events")
